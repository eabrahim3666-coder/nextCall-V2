                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                            ...
                            <span className={`inline-block text-[10px] px-2.5 py-1 rounded-full border whitespace-nowrap text-center ${
                                smsStatus === "rejected"
                                    ...
                                    : smsStatus === "pending"
                                        ? "bg-[#ff4b00]/10 text-[#ff4b00] border-[#ff4b00]/20"
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-2 bg-black border border-white/5 rounded-2xl p-6">
                            <div className="flex items-center justify-between mb-5">
                                <div>
                                    ...
                                    <p className="text-xs text-[#A7ADBB]">Latest calls handled by your AI</p>
                                </div>
                                {recentCalls.length > 0 && (
                                    <Link href="/dashboard/calls" className="text-xs font-medium text-[#ff4b00] hover:text-[#ff4b00]">View all →</Link>
                                )}
                            </div>
                            ...
                            {recentCalls.length === 0 ? (
                                <div className="text-center py-10 border border-dashed border-white/5 rounded-xl">
                                    ...
                                    <p className="text-sm text-[#A7ADBB]">No recent calls yet</p>
                                    ...
                                    {recentCalls.map((call) => (
                                        <div key={call.call_id + "-" + call.created_at} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                                            <div className="flex items-center gap-3 min-w-0 flex-1">
                                                <div className={`flex items-center justify-center w-8 h-8 rounded-lg ${call.sentiment === "Positive" ? "bg-emerald-500/10" : call.sentiment === "Negative" ? "bg-rose-500/10" : "bg-black"}`}>
                                                    <svg className={`w-4 h-4 ${call.sentiment === "Positive" ? "text-emerald-400" : call.sentiment === "Negative" ? "text-rose-400" : "text-[#A7ADBB]"}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
                                                </div>
                                                <div className="min-w-0 flex-1">
                                                    <p className="text-sm font-medium text-white truncate">{call.customer_phone}</p>
                                                    <p className="text-xs text-[#A7ADBB] truncate">{call.summary}</p>
                                                </div>
                                            </div>
                        ...
                        <div className="space-y-6">
                            {activeNumbers.length > 0 && (
                                <div className="bg-black border border-white/5 rounded-2xl p-6">
                                    <h3 className="text-xs font-semibold text-[#A7ADBB] uppercase tracking-wider mb-3">Your AI Numbers</h3>
                            ...
                            <div className="bg-gradient-to-br from-[#ff4b00]/10 to-[#ff4b00]/5 border border-[#ff4b00]/20 rounded-2xl p-6">
                                <h3 className="text-xs font-semibold text-[#ff4b00] uppercase tracking-wider mb-3">💡 Pro Tip</h3>
                                <p className="text-sm text-neutral-300 leading-relaxed">
                                    Forward your business number to your nextCall AI number so every missed call becomes a captured lead.
                                </p>
                                <Link href="/dashboard/docs" className="inline-block text-xs font-medium text-[#ff4b00] mt-3 hover:text-[#ff4b00]">
                                    Learn how →
                                </Link>
                            </div>
                        </div>
