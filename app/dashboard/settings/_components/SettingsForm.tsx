                ...
                <div className="space-y-5">
                    <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-8">
                        ...
                        <div className="space-y-4">
                            {[{ key: "forward_emergency" as keyof RoutingRules, title: "Forward emergency calls to my cell", desc: "When caller says words like 'emergency', 'burst pipe', 'flooding' — AI forwards to your phone", paidOnly: false }, { key: "notify_hot_lead" as keyof RoutingRules, title: "Notify me about hot leads", desc: "Get an in-app notification when AI detects a high-intent lead ready to buy", paidOnly: true }, { key: "sms_missed_call" as keyof RoutingRules, title: "SMS customer if we miss their call", desc: "Auto-send: 'Sorry we missed you! Call us back or reply here.' — Costs ~$0.008 per SMS", paidOnly: true }, { key: "email_followup" as keyof RoutingRules, title: "Send follow-up email after every call", desc: "Customer gets a branded confirmation email with their request details", paidOnly: true }, { key: "daily_summary" as keyof RoutingRules, title: "Daily summary email", desc: "Every morning you get an email with: calls, leads, appointments, and revenue captured", paidOnly: true }, { key: "appointment_reminders" as keyof RoutingRules, title: "Appointment reminders", desc: isTrial ? "Email reminder 24 hours before the appointment" : "SMS 1 hour before + email 24 hours before the appointment", paidOnly: false }, { key: "review_followup" as keyof RoutingRules, title: "Auto review follow-up", desc: "If you forget to mark a job done, we ask the customer for a review 8 hours after their appointment", paidOnly: false }].filter(rule => !rule.paidOnly || !isTrial).map((rule) => (
