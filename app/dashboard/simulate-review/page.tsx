...
export default function SimulateReviewPage() {
  ...
  const handlePost = async () => {
    ...
    try {
      const res = await fetch("/api/reviews/simulate-reply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ review_text: reviewText, review_stars: rating }),
      });
        ...
        <div className="mt-6 rounded-2xl border border-white/[0.08] bg-[#0a0a0a] overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="px-4 pb-4">
            <p className="text-[11px] text-neutral-600 italic">
              This AI-generated reply is ready. You can push it to Google once verified.
            </p>
          </div>
        </div>
      )}
